...
- `src/LocalMediaTransfer.iOS`: Expo SDK 57 React Native application with a
...
- On Expo SDK 57 iOS, do not build upload `Blob`s from `ArrayBuffer` or
...
- The SDK 57 picker uses `react-native-reanimated 4.5.1` and
  `react-native-worklets 0.10.1` for UI-thread drag auto-scroll. Keep movement based on
