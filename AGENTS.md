...
- `src/LocalMediaTransfer.iOS`: Expo SDK 56 React Native application with a
...
- On Expo SDK 56 iOS, do not build upload `Blob`s from `ArrayBuffer` or
...
- The SDK 56 picker uses `react-native-reanimated 4.3.1` and
  `react-native-worklets 0.8.3` for UI-thread drag auto-scroll. Keep movement based on
